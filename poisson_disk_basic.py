...
def check_collision(p, index):
    x, y = index
    collision = False
    for i in range(max(0, x - 2), min(grid_nx, x + 3)):
        for j in range(max(0, y - 2), min(grid_ny, y + 3)):
            if grid[i, j] != -1:
                q = samples[grid[i, j]]
...
def poisson_disk_sample(desired_samples: int) -> int:
    samples[0] = tm.vec2(0.5/2, 0.5)
    grid[int(grid_nx * 0.5), int(grid_ny * 0.5)] = 0
    head, tail = 0, 1
    while head < tail and head < desired_samples:
        ...
        for _ in range(100):
            ...
            if 0 <= new_x[0] < (res[0]/res[1]) and 0 <= new_x[1] < 1:
